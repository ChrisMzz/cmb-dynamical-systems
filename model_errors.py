...
def check_in_range(params):
    ...
    if not (10e-5 <= pi0 <= 1): warnings.warn('pi0 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-5 <= pi1 <= 1): warnings.warn('pi1 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-5 <= pi2 <= 1): warnings.warn('pi2 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-6 <= gamma2 <= 1): warnings.warn('gamma2 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-6 <= gamma3 <= 1): warnings.warn('gamma3 not in range', ParameterRangeWarning, stacklevel=2)
    ...
    if not (10e-5 <= alpha1 <= 1): warnings.warn('alpha1 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-5 <= alpha2 <= 1): warnings.warn('alpha2 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-5 <= alpha3 <= 1): warnings.warn('alpha3 not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-5 <= alpha2b <= 1): warnings.warn('alpha2b not in range', ParameterRangeWarning, stacklevel=2)
    if not (10e-5 <= alpha3b <= 1): warnings.warn('alpha3b not in range', ParameterRangeWarning, stacklevel=2)
